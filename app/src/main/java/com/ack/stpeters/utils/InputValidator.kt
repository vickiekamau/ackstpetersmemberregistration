package com.ack.stpeters.utils
...
class InputValidator {
    ...
    fun getDOB(d: String): String {
        var str = ""
        if (d != null && !d.isEmpty()) {
            val inputPattern = "ddMMyyyy"
    ...
    fun validateDateOfBirth(
        ...
        if (validateRequired(textInputLayout, textInputEditText)) {
            if (isInvalidFormat(Objects.requireNonNull(textInputEditText.text).toString())) {
                showError(textInputLayout!!, INVALID_DATE)
                setErrorWatcher(textInputEditText, textInputLayout)
                return false
            } else if (isNotRequiredAge(textInputEditText.text.toString())) {
                showError(textInputLayout!!, "Min Age is 10 yrs")
                setErrorWatcher(textInputEditText, textInputLayout)
                return false
            }
        }
        return validateRequired(textInputLayout, textInputEditText)
    }

    private fun isNotRequiredAge(value: String): Boolean {
        ...
        val dt2 = DateTime(date)
        return Years.yearsBetween(dt2, DateTime(Date())).getYears() < 10
    }
    ...
    private fun isInvalidFormat(value: String): Boolean {
        ...
        try {
            ...
            date = sdf.parse(value)
            if (value != sdf.format(Objects.requireNonNull(date))) {
                date = null
            }
